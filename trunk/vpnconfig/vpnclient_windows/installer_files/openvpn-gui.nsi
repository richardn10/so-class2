...
!define GUI_CONFIG_DIR    "$PROFILE\switchedon_vpn"
...
Section "OpenVPN GUI" SecGUI
  ...
  CreateDirectory "$PROFILE\switchedon_vpn"
  SetOutPath "$PROFILE\switchedon_vpn"
...
Section "My Certificate Wizard" SecMYCERT
  ...
  WriteINIStr "$INSTDIR\bin\mycert.ini" "openvpn" "template" "$INSTDIR\sample-config\template.ovpn" 
  WriteINIStr "$INSTDIR\bin\mycert.ini" "paths" "dir" "$PROFILE\switchedon_vpn"
